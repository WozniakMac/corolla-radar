...
Po każdym skanie aplikacja porównuje aktualne TOP 10 z poprzednim. Zmiana składu, kolejności, łącznej punktacji lub którejkolwiek składowej oceny generuje jedno zbiorcze powiadomienie ntfy. Każdy wiersz zawiera tylko pozycję, zmianę pozycji (`↑`, `↓`, `→` lub `NOWE`), punkty, skrót zmiany wyniku i link do szczegółów auta w aplikacji. Pierwszy skan tylko ustala punkt odniesienia. Zapisane filtry ograniczają ranking używany w powiadomieniu. Adres tematu konfiguruje `NTFY_URL`, a bazowy adres aplikacji dla linków — `APP_PUBLIC_URL` (domyślnie `http://192.168.2.47:4174`).
